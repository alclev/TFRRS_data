...
def convert_mark(mark):
    """
    Convert a raw data to understandable metrics
    """
    global unit
...
def write_to_csv(map):
    ...
    @return None
    """
    with open("lehigh_athletes.csv", "w", newline='') as csvfile:
...
def scrape():
    """
    Perform the main scraping functionality of the program.
    """
    print("Scraping...")
    response = requests.get("https://www.tfrrs.org/teams/tf/PA_college_m_Lehigh.html")
...
def main():
    ...
    """
    # scrape()
    map = load_file("lehigh_athletes.csv")
    interpret(map)
